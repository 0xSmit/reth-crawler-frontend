...
export function getClientVersionDistribution(
  clientResult: any,
  clientFilter: string
): Promise<ClientVersionDistribution[]> {
  if (!clientFilter) {
    return Promise.resolve([]);
  }
  if (typeof clientResult === 'string') {
...
export function getLanguageVersionDistribution(
  clientResult: any,
  clientFilter: string
): Promise<LanguageVersionDistribution[]> {
  if (typeof clientResult === 'string') {
    clientResult = JSON.parse(clientResult);
  }
  const filteredData = clientResult.filter((data: { client_version: string }) => {
